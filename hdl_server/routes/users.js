    ...
    if (result.length == 0) {
      ...
      pool.query(reg, [phone, upwd, uname], (err, result) => {
        ...
        if (result.affectedRows > 0) {
          //注册vip
          req.session.uid = result.insertId; //记录登录状态
          user.uid = result.insertId; //保存注册的用户id
          pool.query(
            "INSERT INTO vip(u_id,vip_regtime) VALUES(?,CURRENT_TIMESTAMP)",
            [result.insertId],
            (err, result) => {
              if (err) throw err;
              ...
              if (result.affectedRows > 0) {
                pool.query(
                  "SELECT date_format(vip_regTime,'%Y%m%d') FROM vip WHERE vip_id = ?",
                  [user.vip_id],
                  (err, result) => {
                    if (err) throw err;
                    if (result.length > 0) {
                      ...
                      res.send({ code: 2, msg: "register successfully", user }); //注册成功
                    }
                  }
                );
              } else {
      ...
      pool.query(sql, [phone, upwd], (err, result) => {
        if (err) throw err;
        if (result.length > 0) {
          req.session.uid = result[0].id; //记录登录状态
          ...
          pool.query(
            "SELECT vip_area,date_format(vip_regTime,'%Y%m%d'),vip_id,vip_money,vip_value FROM vip WHERE u_id = ?",
            [user.uid],
            (err, result) => {
              if (err) throw err;
              ...
              res.send({ code: 1, msg: "login successfully", user }); //登录成功
            }
          );
        } else {
    ...
    (err, result) => {
      if (result[0].upwd == md5(req.body.oldPwd)) {
        pool.query(
          ...
          [req.body.newPwd, req.session.uid],
          (err, result) => {
            if (err) throw err;
            if (result.affectedRows > 0) {
...
router.get("/logout", (req, res) => {
  ...
  if (!uid) {
    res.send({ code: -4, msg: "logout successful" });
    return;
  } else {
    res.send({ code: 4, msg: "logout failed" });
  }
});
