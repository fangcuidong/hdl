...
router.get("/new", (req, res) => {
  ...
  pool.query(
    "select u_id,label,content,post_time,origin,likes,comments,images,uname,say,user_img from posts inner join user on user.id = posts.u_id order by posts.post_time limit ?,?",
    [pageNumber, pageSize],
    (err, result) => {
      if (err) throw err;
      if (result.length > 0) {
...
router.get("/myfollow", (req, res) => {
  pool.query(
    ...
    [req.session.uid, Number(req.query.pageNumber), Number(req.query.pageSize)],
    (err, result) => {
      if (err) throw err;
      if (result.length > 0) {
        res.send({ code: 1, result });
